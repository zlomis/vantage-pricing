...
//  - New cell patches: B9 deal_structure, B58 contingency, B68/B77 salary multipliers,
...
//  - Removed stale cached-value patches (MS subtotals, IO B4-B12, SO C21) — fullCalcOnLoad="1"
//    in baseline workbook.xml makes Excel recalculate on open, eliminating drift risk
//  - Removed setNum('B22') override — baseline B22 is now =ROUND(B25/B21,0) formula
//  - Removed Cover setCoverStr writes — Cover B8-B12 are now =Assumptions!Bn formulas
...
// vantageCalcCC — Clinical Costs line-item replica of baseline_v2.xlsx Clinical Costs sheet.
// MUST mirror CC!E14, E16, E34, E36, E58, E60, E63, E65, F65 exactly.
// Per-patient cost basis is a STATIC default in the baseline (healthy-vol Phase 1):
//   - Screening: $136/patient screened (sum of rows 5-12 base costs × multipliers)
//   - Treatment: $1,234/patient enrolled (sum of rows 19-30)
// For non-healthy-vol indications (oncology, etc.), users should edit CC sheet directly.
function vantageCalcCC(A) {
  const subj    = Number(A.subj_enroll)||0;
  const screen  = Math.round(subj * 1.3);                // matches B24 formula
  ...
  const e56 = 0;                              // Cohort B not used in baseline
